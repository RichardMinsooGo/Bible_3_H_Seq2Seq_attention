...
def train_step(inp, tar, enc_hidden):
    ...
    with tf.GradientTape() as tape:
        ...
        '''
        T4. Create decoder Input and Ground Truth
        '''        
        dec_input = tar[ : , :-1 ]   # Ignore <end> token
        real = tar[ : , 1: ]         # ignore <start> token
        
        '''
        T5. Set the AttentionMechanism object with encoder_outputs
        ''' 
        decoder.attention_mechanism.setup_memory(enc_output)
        
        '''
        T6. Create AttentionWrapperState as initial_state for decoder
        ''' 
...
def evaluate(text):
    ...
    '''
    E5. Instantiate BasicDecoder object
    '''
    
    decoder_instance = tfa.seq2seq.BasicDecoder(cell=decoder.rnn_cell, sampler=greedy_sampler, output_layer=decoder.fc)
    
    '''
    E6. Setup Memory in decoder stack
    '''
    decoder.attention_mechanism.setup_memory(enc_output)
    
    '''
    E7. Set decoder_initial_state
    '''
    decoder_initial_state = decoder.build_initial_state(inference_batch_size, [enc_state_h, enc_state_c], tf.float32)
    
    '''
    E8. Ensure decoding step is output of embedding layer
    '''
