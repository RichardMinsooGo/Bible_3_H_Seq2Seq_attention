...
class Decoder(Model):
    ...
    def call(self, dec_input, enc_output):
        # dec_input = Decoder input, shape = (BATCH_SIZE, 1)
        # enc_output = final hidden state from Encoder, shape = (BATCH_SIZE, hid_dim)

        # dec_input shape after passing through embedding == (BATCH_SIZE, 1, embedding_dim)
        x = self.embedding(dec_input)
        
        output, state = self.gru(x, initial_state=enc_output)
...
def train_step(inp, tar, enc_hidden):
    ...
    with tf.GradientTape() as tape:
        '''
        T2. Encoder Output / Hidden State
        '''
