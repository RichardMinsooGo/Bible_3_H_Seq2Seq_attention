...
class Encoder(Model):
    def __init__(self, vocab_size, embedding_dim, enc_units, BATCH_SIZE):
        ...
        for i in range(self.num_bi_layers):
            self.sub_layer[i] = Bidirectional(LSTM(self.enc_units,
        ...
        for i in range(self.num_bi_layers, self.num_layers):
            self.sub_layer[i] = LSTM(self.enc_units,
                                     return_sequences=True,
                                     return_state=True,
                                     recurrent_initializer='glorot_uniform')

    def call(self, inp_batch):
        x = self.embedding(inp_batch)
        for i in range(self.num_bi_layers):
            x, h_fw, c_fw, h_bw, c_bw = self.sub_layer[i](x)
        
        self.hidden_hs[i] = self.final_h(tf.concat([h_fw, h_bw], axis=1))
        self.hidden_cs[i] = self.final_c(tf.concat([c_fw, c_bw], axis=1))
...
print(len(sample_h))
print ('Encoder h vector shape: (batch size, hid_dim) {}'.format(sample_h[0].shape))
print ('Encoder c vector shape: (batch size, hid_dim) {}'.format(sample_c[0].shape))
...
class Decoder(Model):
    def __init__(self, vocab_size, embedding_dim, dec_units, BATCH_SIZE):
        ...
        for i in range(self.num_layers):
            self.sub_layer[i] = LSTM(self.dec_units,
                                     return_sequences=True,
                                     return_state=True,
                                     recurrent_initializer='glorot_uniform'
                                     )
    ...
    def call(self, dec_input, hidden_hs, hidden_cs, enc_outputs):
...
print('Decoder_h shape: ', dec_h[0].shape)
print('Decoder_c shape: ', dec_c[0].shape)
...
for epoch in range(0,50):
    ...
    '''
    S1. [PASS] Initialize Encoder hidden state
    '''
...
def evaluate(text):
    ...
    '''
    E2. [PASS] Initialize encoder hidden state as Zeros
    '''
    ...
    '''
    enc_out, enc_state_h, enc_state_c = encoder(inputs)
...
def translate_batch(test_dataset):
    ...
    with open('output_text.txt', 'w') as f:
        for (inputs, targets) in test_dataset:
            ...
            '''
            B3. [PASS] Initialize encoder hidden state as Zeros
            '''
            ...
            '''
            enc_output, dec_h, dec_c = encoder(inputs)
